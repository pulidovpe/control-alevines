...
<?php
  ...
  $iAnchoTabular = 10;
	...
	if($alevines>0)  {
		...
		$pdf->SetFont('Arial','',9);
		$pdf->Cell($iAnchoTabular);
		$pdf->Cell(70,6,"Alevines de Cachama",1,0,'L');
	...
	if($juveniles>0)  {
		...
		$pdf->SetFont('Arial','',9);
		$pdf->Cell($iAnchoTabular);
		$pdf->Cell(70,6,"Juveniles de Cachama",1,0,'L');
	...
	while($MostrarFila2 = mysql_fetch_array($Resultado2)) {
		if($MostrarFila2['forma_pago']=='Deposito') {	  	
	  	$pdf->SetFont("Arial","B",11);
	  	$pdf->Cell($iAnchoTabular);
	  	$pdf->Cell(0,5,"-DEPOSITO REALIZADO EN LA CUENTA CORRIENTE: 0163-0228-72-2283002826",0,0,"L");
  		$pdf->Ln();
  		$pdf->SetFont("Arial","B",11);
	  	$pdf->Cell($iAnchoTabular);
  		...
  		$pdf->SetFont("Arial","B",11);
	  	$pdf->Cell($iAnchoTabular);
	  	$pdf->Cell(0,5,"SEGUN DEPOSITO NUMERO: ".$MostrarFila2['num_cheqdep']." DE FECHA: ".$MostrarFila2['fecha']." POR Bs: ".$MostrarFila2['monto'],0,0,"L");
  		$pdf->Ln();
  	}
  	if($MostrarFila2['forma_pago']=='Cheque') {	  	
	  	$pdf->SetFont("Arial","B",11);
	  	$pdf->Cell($iAnchoTabular);
	  	$pdf->Cell(0,5,"-PAGO CON CHEQUE EN LA OFICINA DE COBROS PARA SER DEPOSITADO EN: ",0,0,"L");
  		$pdf->Ln();
  		$pdf->SetFont("Arial","B",11);
	  	$pdf->Cell($iAnchoTabular);
	  	$pdf->Cell(0,5,$texto1,0,0,"L");
  		...
  		$pdf->SetFont("Arial","B",11);
	  	$pdf->Cell($iAnchoTabular);
	  	$pdf->Cell(0,5,"NUMERO DE CHEQUE: ".$MostrarFila2['num_cheqdep']." DE FECHA: ".$MostrarFila2['fecha']." POR Bs: ".$MostrarFila2['monto'],0,0,"L");
  		$pdf->Ln();
  	}
  	if($MostrarFila2['forma_pago']=='Efectivo') {	  	
	  	$pdf->SetFont("Arial","B",11);
	  	$pdf->Cell($iAnchoTabular);
	  	$pdf->Cell(0,5,"-PAGO HECHO EN EFECTIVO EN LA OFICINA DE COBROS DE LA ESTACION POR Bs: ".$MostrarFila2['monto'],0,0,"L");
  		$pdf->Ln();
  	}
	}	
