...
if(empty($cedula)) { 
	...
	if($res5==false || $res4==false || $res3==false || $res2==false) {
		...
		if($total>0) {
			$p3 = $total;
			// Se facturan los empaques y se procesa la donacion			
			...
			if($res0==false) {
				...
				if(!empty($cant1) || !empty($cant2) || !empty($cant3)) {
					// Si hay donativos, se procesan
					$sql2="INSERT INTO donaciones (id_donativo,fecha_don,cedula,cant_alev1,cant_alev2,cant_juve) 
					...
					if($res1==false) {
						...
						echo "06".$message;
						//echo "06"." Error al donar despues de facturar los empaques"." Cant1: ".$cant1." Cant2:".$cant2;
						// Error al incluir donativo
					} else {
						echo "09"." o este 1";
			...
			if($res1==false) {
		    	...
		    	$message .= 'Cual consulta: ' . $sql2;
				echo "06".$message;
				//echo "06"." Error al donar"." Cant1: ".$cant1." Cant2:".$cant2;
				// Error al incluir donativo
			} else {
				echo "04"." o este 3";
				// Grabacion exitosa!
