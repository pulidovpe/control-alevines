...
$sql1="SELECT * FROM reposicion, productor WHERE reposicion.cedula = productor.cedula AND reposicion.id_reposicion=".$id;
...
<?php
  ...
  $aFecha = explode("-",$MostrarFila['fecha_rep']);
  ...
  $pdf->Ln(13);
  $pdf->Cell(69);  
  $pdf->write(0,utf8_decode("ACTA DE REPOSICIÓN"));
  $pdf->Ln(15);
  // Numero de factura y fecha
  $texto3 = utf8_decode("Número de Reposicion: 0");
  $iAnchoTabular = 125;   
  $pdf->Ln();
   $pdf->SetFont("Arial","B",11);
  $pdf->Cell($iAnchoTabular);
  $pdf->Cell (0,5,$texto3.$MostrarFila['id_reposicion']."/".$anio,20,0,"L");
  $pdf->Ln();
  $pdf->SetFont("Arial","B",11);
  $pdf->Cell($iAnchoTabular);
  ...
  $pdf->SetFont("Arial","B",11);
  $pdf->Cell($iAnchoTabular);
  $pdf->Cell(0,5,utf8_decode("Se levanta la presente Acta para dejar constancia de la reposición por la cantidad de: ").$MostrarFila['cant_alev1']);
  $pdf->Ln();
  $pdf->Cell($iAnchoTabular);
  $pdf->Cell(0,5,utf8_decode("Se levanta la presente Acta para dejar constancia de la reposición por la cantidad de: ").$MostrarFila['cant_alev2']);
  $pdf->Ln();
  $pdf->Cell($iAnchoTabular);
  $pdf->Cell(0,5,utf8_decode("Alevines de Cachama en atención en lo dispuesto en el articulo Tercero Literal B de"));
  $pdf->Ln();
  $pdf->Cell($iAnchoTabular);
  $pdf->Cell(0,5,utf8_decode("de los estatutos internos de la Fundación Estación Piscícola San Fernando."));
  $pdf->Ln();
  ...
  $pdf->SetFont("Arial","B",11);
  $pdf->Cell($iAnchoTabular);
  $pdf->Cell(0,5,"Nombre del Productor: ".$MostrarFila['nombres']." ".$MostrarFila['apellidos']);
  $pdf->Ln();
  $pdf->SetFont("Arial","B",11);
  $pdf->Cell($iAnchoTabular);
  ...
  $pdf->Cell(10); 
  $pdf->Cell(0,5,utf8_decode("Lic. José Valera"),0,0,"L");
  ...
  $texto2 = utf8_decode("Fundación Estación Piscícola San Fernando, a 4 Km.del puente María nieves, Biruaca Edo. Apure");
  $pdf->Cell(20);
